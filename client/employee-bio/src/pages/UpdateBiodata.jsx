...
export default function UpdateBiodata() {
  ...
  const fetchCurrentBiodata = async () => {
    try {
      const token = cookieHelper.getAccessToken();

      if (!id) {
        throw new Error("User ID tidak ditemukan di URL");
      }
      ...
      const isOwn = currentUserResponse.data.id.toString() === id.toString();
      setIsOwnProfile(isOwn);

      let biodata, userIdForUpdate;

      // Baik profil sendiri maupun profil lain memiliki struktur yang sama:
      // Objek User dengan array Biodata
      if (userData.Biodata && userData.Biodata.length > 0) {
        biodata = userData.Biodata[0];
        userIdForUpdate = userData.id; // User ID from user object
      } else {
        throw new Error("Tidak ada biodata ditemukan untuk user ini");
      }

      if (biodata) {
        setBiodataId(biodata.id);
        setUserId(userIdForUpdate);

        // Format birth_date untuk input
        const birthDate = new Date(biodata.birth_date);
        const formattedBirthDate = birthDate.toISOString().split("T")[0];

        setFormData({
          applied_position: biodata.applied_position || "",
          full_name: biodata.full_name || "",
          id_number: biodata.id_number || "",
          birth_place: biodata.birth_place || "",
          birth_date: formattedBirthDate,
          gender: biodata.gender || "",
          religion: biodata.religion || "",
          blood_type: biodata.blood_type || "",
          marital_status: biodata.marital_status || "",
          address_id: biodata.address_id || "",
          address_current: biodata.address_current || "",
          personal_email: biodata.personal_email || "",
          phone_number: biodata.phone_number || "",
          emergency_contact: biodata.emergency_contact || "",
          willing_to_relocate: biodata.willing_to_relocate || false,
          expected_salary: biodata.expected_salary || "",
          skills: "", // Skills akan dihandle terpisah jika diperlukan
        });

        setEducations(biodata.Education || []);
        setTrainings(biodata.Trainings || []);
        setWorkExperiences(biodata.WorkExperiences || []);
      }
    } catch (error) {
      console.log(error);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!id) {
      Swal.fire({
        title: "Gagal",
        text: "User tidak ditemukan untuk diperbarui.",
        icon: "error",
      });
      return;
    }
    ...
    try {
      ...
      Swal.fire({
        title: "Berhasil",
        text: "Data biodata berhasil diperbarui!",
        icon: "success",
      });

      // Navigasi kembali ke halaman profil yang sesuai
      if (isOwnProfile) {
        navigate("/");
      } else {
    ...
    return (
      <div className="container py-5">
        <div className="text-center py-5">
          <div className="spinner-border text-primary" role="status" style={{ color: "var(--bawaslu-red) !important" }}>
            <span className="visually-hidden">Memuat...</span>
          </div>
          <p className="mt-3 text-muted">Memuat data biodata saat ini...</p>
        </div>
      </div>
    );
  }
          ...
          <div className="d-flex justify-content-between align-items-center mb-4">
            <div>
              <h2 className="text-dark mb-1">
                {isOwnProfile ? "Perbarui Biodata" : "Perbarui Biodata Pengguna"}
              </h2>
              <p className="text-muted mb-0">
                {isOwnProfile
                  ? "Perbarui informasi pribadi dan riwayat hidup Anda"
                  : "Perbarui informasi pribadi dan riwayat hidup pengguna"}
              </p>
            </div>
            <Link to={isOwnProfile ? "/" : `/profiles/${id}`} className="btn btn-outline-secondary">
              Batal
            </Link>
          </div>
          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className="card border-0 shadow-sm mb-4">
              ...
              <div className="card-body">
                ...
                <div className="row">
                  <div className="col-md-6 mb-3">
                    <label className="form-label text-dark">Alamat Email Pribadi *</label>
                    <input
                      type="email"
                      name="personal_email"
                      className="form-control"
              ...
              <div className="card-body">
                {educations.length === 0 && (
                  <p className="text-muted">
                    Belum ada data pendidikan. Klik "+ Tambah Pendidikan" untuk menambah.
                  </p>
                )}
            ...
            <div className="card border-0 shadow-sm mb-4">
              ...
              <div className="card-body">
                {trainings.length === 0 && (
                  <p className="text-muted">
                    Belum ada data pelatihan. Klik "+ Tambah Pelatihan" untuk menambah.
                  </p>
                )}
                      ...
                      <div className="col-md-3 mb-3">
                        ...
                        <div className="form-check mt-2">
                          <input
                            type="checkbox"
                            className="form-check-input"
                            checked={training.has_certificate || false}
                            onChange={(e) =>
                              handleTrainingChange(
                                index,
                                "has_certificate",
            ...
            <div className="card border-0 shadow-sm mb-4">
              ...
              <div className="card-body">
                {workExperiences.length === 0 && (
                  <p className="text-muted">
                    Belum ada data pengalaman kerja. Klik "+ Tambah Pengalaman Kerja" untuk menambah.
                  </p>
                )}
            ...
            <div className="card border-0 shadow-sm mb-4">
              ...
              <div className="card-body">
                ...
                <textarea
                  ...
                  className="form-control"
                  rows="4"
                  placeholder="Contoh: Pengawasan Pemilu, Hukum Tata Negara, Microsoft Office, PHP, dll."
